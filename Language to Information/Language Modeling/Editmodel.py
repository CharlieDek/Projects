...
class EditModel(object):
    ...
    if len(word) < 0:
      return []
        ...
        for i in xrange(1, len(word) + 1 ):
          ...
          for l in EditModel.ALPHABET:
            correctLetters = word[i-1] + l
            corruptLetters = word[i - 1]
            correction = "%s%s%s" % (preceding, l, following)
            ret.append(Edit(correction, corruptLetters, correctLetters))
    return ret
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(2, len(word) ):
      ...
      correction = "%s%s%s" % (word[1:i - 1], correctLetters, word[i+1:])
      ret.append(Edit(correction, corruptLetters, correctLetters))
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(2, len(word) + 1 ):
      preceding = word[1:i-1]
      following = word[i:]
      for l in EditModel.ALPHABET:
        corruptLetters = word[i - 1]
        correction = "%s%s%s" % (preceding, l, following)
        if (correction != word[1:]):
          ret.append(Edit(correction, corruptLetters, l))
    return ret
